...
async function getSkills(): Promise<Skill[]> {
  const res = await fetch(`${APP_URL}/api/skills`, { cache: 'no-store' });
  if (!res.ok) {
  ...
  if (error) {
    return (
      <div className="text-destructive-foreground bg-destructive p-4 rounded-md">
        ...
        <p>{error}</p>
        <p>Please ensure the API endpoint at <code className="text-sm bg-destructive-foreground/20 px-1 rounded">{APP_URL}/api/skills</code> is running and accessible.</p>
      </div>
    );
  }
